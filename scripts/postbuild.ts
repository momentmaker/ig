...
import { buildOgImages, buildHomeOgImage } from './build-og-images'
...
async function main(): Promise<void> {
  ...
  const home = await buildHomeOgImage('data/manifest.json', OG_CACHE_DIR)
  ...
  console.log(`  home composite: ${home.written ? 'wrote' : `skipped (${home.reason ?? 'unknown'})`}`)
